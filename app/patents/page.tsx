import { Metadata } from "next";
import { Header, Footer, SectionPatents } from "@/components/sections";
import { PatentsHero, PatentsTimeline, PatentsApplications } from "@/components/patents";
import { patentsFrItems } from "@/lib/content-bilingual";

export const metadata: Metadata = {
  title: "Brevets INPI | IA Solution",
  description: `${patentsFrItems.length} brevets déposés à l'INPI : FR2514274 (authentification cognitive), FR2514546 (Celestial Entropy Layer), FR2515560. Protection 20 ans.`,
};

export default function PatentsPage() {
  return (
    <>
      <Header />
      <main>
        <PatentsHero />
        <SectionPatents />
        <PatentsTimeline />
        <PatentsApplications />
      </main>
      <Footer />
    </>
  );
}
